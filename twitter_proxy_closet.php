...
class TwitterProxy_search {
	 ...
	 *	@param	string	$consumer_secret			Consumer secret				('API secret' on https://apps.twitter.com)
	  *	@param	string	$id							id (http://gettwitterid.com/)
	  *	@param	string	$q							query (http://gettwitterid.com/)
	...
	public function __construct($oauth_access_token, $oauth_access_token_secret, $consumer_key, $consumer_secret, $lat , $long) {
		...
		$this->config = array_merge($this->config, compact('oauth_access_token', 'oauth_access_token_secret', 'consumer_key', 'consumer_secret', 'lat' ,'long'));

		//$this->whitelist['trends/place.json?user_id=' . $this->config['user_id'] . '&screen_name=' . $this->config['screen_name'] . '&count=' . $this->config['count']] = true;
		
		$this->whitelist['trends/closest.json?lat=' . $this->config['lat'] . '&long=' .$this->config['long']] = true;
